...
def sgdRandom(
    ...
    for _ in range(n_iter):
        ...
        for start in range(0, n_obs, batch_size):
            ...
            point = random.randrange(0, np.size(xy, 0), 1)
            x_batch, y_batch = xy[point, :-1], xy[point, -1:]
...
print(sgd(ssr_gradient, x, y, n_vars=2, learn_rate=0.0001,
    decay_rate=0.8, batch_size=3, n_iter=100_000, random_state=0))

x = np.array([5, 15, 25, 35, 45, 55])
y = np.array([5, 20, 14, 32, 22, 38])
print(sgdRandom(ssr_gradient, x, y, n_vars=2, learn_rate=0.0001,
    decay_rate=0.8, batch_size=3, n_iter=100_000, random_state=0))
print(sgd(ssr_gradient, x, y, n_vars=2, learn_rate=0.0001,
    decay_rate=0.8, batch_size=3, n_iter=100_000, random_state=0))
